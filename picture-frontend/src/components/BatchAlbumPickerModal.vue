<template>
  <BaseModal :model-value="modelValue" title="加入到相册" @update:model-value="v => emit('update:modelValue', v)">
    <p class="text-sm text-gray-500 mb-3">已选 {{ pictureIds.length }} 张图片，选择目标相册：</p>
    ...
    <div v-else class="max-h-80 overflow-y-auto space-y-2">
      ...
      >
        ...
        <button
          @click="select(album)"
          :disabled="submitting"
          class="px-3 py-1 text-xs font-medium text-blue-600 bg-blue-50 rounded-full hover:bg-blue-100 transition flex-shrink-0 disabled:opacity-50"
        >
          加入
        </button>
      </div>
...
const select = async (album) => {
  if (!props.pictureIds.length) return
  submitting.value = true
  try {
    const res = await batchAddPictures(album.id, props.pictureIds)
    showToast(summarizeBatchResult(`加入「${album.name}」`, res?.result))
    emit('added', res?.album)
    close()
  } catch (e) {
